...
export default function AppShell() {
	...
	const handleOutsideClick = (event) => {
		if (
			rightSidePanelRef.current &&
			!rightSidePanelRef.current.contains(event.target)
		) {
			setRightPanelOpen(false);
		}
		if (
			leftSidePanelRef.current &&
			!leftSidePanelRef.current.contains(event.target)
		) {
				...
				{leftPanelOpen && (
					...
					>
						...
						<div className="space-y-6 px-3">
							...
							<div>
								<div className="text-xs font-medium text-muted-foreground/70 px-3 mb-2">
									Dashboards
								</div>
								<div className="space-y-0.5 relative">
									<SidebarItem
				...
				<header className="h-16 border-b border-border flex items-center justify-between px-6 bg-background/50 backdrop-blur-sm sticky top-0 z-10 @container">
					<div className="flex items-center gap-4 text-sm">
						...
						<div className="flex items-center gap-2 text-muted-foreground">
							...
							<span className="text-muted-foreground/40 hidden @xl:block">
								/
							</span>
							<span className="text-foreground font-medium hidden @xl:block">
								Default
							</span>
						</div>
					</div>
					...
					<div className="flex items-center gap-2">
						<div className="relative hidden @xl:block mr-2">
							...
							/>
							<span className="absolute right-2 top-1.5 text-xs text-muted-foreground/60 p-0.5 border border-border rounded bg-background/50">
								⌘/
							</span>
						</div>
						...
						<div className="space-y-4">
							...
							<div className="space-y-0">
								<Button
									variant="ghost"
									size="xl"
									className="justify-start group w-full px-3 text-left"
								>
