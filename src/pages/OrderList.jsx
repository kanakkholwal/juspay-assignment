...
export default function OrderList() {
    ...
    const togglePendingFilter = () => {
        setColumnFilters((prev) => {
            const hasPendingFilter = prev.some((filter) => filter.id === "status" && filter.value === "Pending");
            return hasPendingFilter ? [] : [{ id: "status", value: "Pending" }];
        });
    };
        ...
        <div className="space-y-4 h-full flex flex-col">
            ...
            <div className="flex flex-wrap justify-between items-start sm:items-center gap-4 bg-card p-2 rounded-lg border border-border/40 @container/toolbar">
                <div className="flex items-center gap-2">
                    <Button variant="ghost" size="icon_sm" onClick={handleAddOrder}>
                        <Icon name="plus" />
                    </Button>
                    <Button
                        variant={columnFilters.length > 0 ? "default_soft" : "ghost"}
                        size="icon_sm"
